...
def merge_small_regions(
    ...
    thresholds = (
        np.full(region_count + 1, min_region_pixels, dtype=np.int64)
        if region_min_pixels is None
        else np.asarray(region_min_pixels, dtype=np.int64).copy()
    )
    if len(thresholds) < region_count + 1:
        raise ValueError("region_min_pixels ne couvre pas toutes les régions")
    groups = (
        np.zeros(region_count + 1, dtype=np.int8)
        if region_groups is None
        else np.asarray(region_groups, dtype=np.int8).copy()
    )
    if len(groups) < region_count + 1:
        raise ValueError("region_groups ne couvre pas toutes les régions")
    ...
    queue = [
        ...
        if sizes[region_id] < thresholds[region_id]
    ]
    heapq.heapify(queue)
    events: list[MergeEvent] = []
    forced_merges = 0
    ...
    while queue:
        ...
        same_group_neighbors = [
            ...
            if active[target] and groups[target] == groups[source]
        ]
        allowed_targets = set(same_group_neighbors or graph[source].keys())
        ...
        if sizes[target] < thresholds[target]:
