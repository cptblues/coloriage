...
def _make_print_preview(result: PipelineResult, colored: bool) -> Image.Image:
    ...
    line_width_px = max(
        1,
        round(result.config.line_width_mm / geometry.mm_per_pixel),
    )
    if colored:
        source = _with_boundaries(
            result.merged_rgb,
            result.region_labels_after,
            line_width=line_width_px,
        )
    else:
        source = _contours_only(
            result.region_labels_after,
            line_width=line_width_px,
        )
    panel = Image.fromarray(source, "RGB").resize(
        image_size,
